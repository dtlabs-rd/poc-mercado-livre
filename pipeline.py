...
while True:
    ...
    results = {
        "valid": plate in person['plates'],
        "name": person['name'],
        "plate_text": plate,
        "plate_bbox": alpr_result[0]['detection'].tolist(),
        "face_bbox": face_recognition_result[0].face['bbox'].tolist(),
        "face_kpts": face_recognition_result[0].face['kps'].tolist(),
        "alpr_frame": base64.b64encode(cv2.imencode('.jpg', alpr_frame)[1].tobytes()).decode('utf-8'),
        "face_recognition_frame": base64.b64encode(cv2.imencode('.jpg', face_recognition_frame)[1].tobytes()).decode('utf-8'),
    }
